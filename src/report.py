...
def main():
    ...
    df = df[~(df.apply(lambda r: (r["model"], r["horizon"]) in tuned and r["tag"] == "full",
                       axis=1))]
    num = df.select_dtypes("number")
    df = pd.concat([df.drop(columns=num.columns), num.round(4)], axis=1)

    agg = (df.groupby(["model", "horizon"])
             .agg(mae=("test_mae", "mean"), mae_sd=("test_mae", "std"),
                  rmse=("test_rmse", "mean"), mape=("test_mape", "mean"),
                  smape=("test_smape", "mean"), mae_peak=("test_mae_peak", "mean"),
                  negsens=("test_negsens_rate", "mean"),
                  ramp_viol=("test_ramp_viol", "mean"), n=("seed", "count"))
             .reset_index())
    ...
    if len(ab):
        ...
        print(ab[["horizon", "lam_thermal", "lam_smooth", "test_mae", "test_rmse",
                  "test_negsens_rate", "test_ramp_viol"]].to_string(index=False))
    ...
    colors = {"lstm": "tab:gray", "tide": "tab:blue", "pinn": "tab:red", "pg_tide": "tab:green"}
    for h, gg in sub.groupby("horizon"):
        for _, r in gg.iterrows():
            ax.scatter(r.negsens * 100, r.mae, marker=markers[r.model], s=60,
                       c=colors[r.model], edgecolors="k",
    ...
    if len(ab):
        ...
        ax.set_xticklabels([f"({a},{b})" for a, b in
                            ab[ab.horizon == ab.horizon.min()][["lam_thermal", "lam_smooth"]].values],
                           fontsize=8)
