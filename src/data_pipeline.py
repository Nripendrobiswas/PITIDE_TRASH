...
HIST_CHANNELS = ["T", "RH", "SP", "h_sin", "h_cos", "dw_sin", "dw_cos",
                 "dy_sin", "dy_cos", "weekend", "demand"]
...
def load_frame(csv_path):
    ...
    spike = ((demand < 0.35 * med) | (demand > 1.6 * med) | (demand > 17500)) & med.notna()
...
def build_feature_frame(df, stats=None):
    ...
    if stats is None:
        ...
        stats = dict(mean=mean.to_numpy(float), std=std.to_numpy(float),
                     cdh_mean=cdh_mean, cdh_std=cdh_std,
                     static_mean=s_mean.to_numpy(float), static_std=s_std.to_numpy(float),
                     ramp_mw=ramp_mw)

    X = ((feats.to_numpy(float) - stats["mean"]) / stats["std"]).astype(np.float32)
    S = ((static.to_numpy(float) - stats["static_mean"]) / stats["static_std"]).astype(np.float32)
    return X, S, stats


def _range_idx(idx, start, end):
    return int(idx.searchsorted(pd.Timestamp(start))), int(idx.searchsorted(pd.Timestamp(end), side="right"))
...
def build_dataset(cfg: Config, horizon: int):
    ...
    full = sliding_window_view(X, total, axis=0).transpose(0, 2, 1)  # (M, total, C) zero-copy view
    try:
    ...
    def split2(start, end, stride):
        ...
        return dict(xp=w[:, :L], xf=w[:, L:, :N_FUT], y=w[:, L:, IDX_DEMAND],
                    s=S[sel], t0=idx[sel])
    ...
    for d in (tr, va, te):
        ...
        d["hot"] = (d["xp"][:, :, IDX_T] * stats["std"][IDX_T] + stats["mean"][IDX_T]) > cfg.hot_threshold

    return dict(train=tr, val=va, test=te, stats=stats, horizon=H,
                dmean=float(dmean), dstd=float(dstd), ramp_scaled=float(ramp_scaled))
...
if __name__ == "__main__":
    ...
    print("stats:", {k: (np.round(v, 3) if isinstance(v, np.ndarray) else round(v, 3))
