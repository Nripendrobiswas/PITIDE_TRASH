    ...
    if key not in _DS_CACHE:
        ...
        if quick:
            for part in ("train", "val", "test"):
                m = min(len(ds[part]["y"]), {"train": 6000, "val": 300, "test": 300}[part])
...
def get_model(name, ds, cfg, hp=None):
    hp = hp or {}
    kw = dict(n_hist=dp.N_HIST, n_fut=dp.N_FUT, n_static=3, l_hist=cfg.l_hist,
              h_out=ds["horizon"], stats=ds["stats"],
...
def tensors(split):
    return TensorDataset(*[torch.from_numpy(np.asarray(split[k]))
                           for k in ("xp", "xf", "y", "s")])


def run_one(cfg, name, horizon, seed, lam_t=0.0, lam_s=0.0, quick=False,
            epochs=None, save_preds=False, verbose=True, hp=None):
    hp = hp or {}
    t0 = time.time()
    ds = get_dataset(cfg, horizon, quick)
    row = dict(model=name, horizon=horizon, seed=seed, lam_thermal=lam_t,
               lam_smooth=lam_s, tag="quick" if quick else "full")
    ...
    if name == "seasonal_naive":
        ...
        row["train_sec"] = 0.0
        if verbose:
            print(f"{name:15s} h={horizon:3d} s={seed}           "
    ...
    phys = PhysicsLoss(lam_t, lam_s, ds["stats"], ds["ramp_scaled"]) if name in PHYSICS_MODELS else None
    opt = torch.optim.Adam(model.parameters(), lr=hp.get("lr", cfg.lr),
    ...
    v_xp, v_xf, v_s, v_y = [torch.from_numpy(np.asarray(ds["val"][k])).to(dev)
                            for k in ("xp", "xf", "s", "y")]
    ...
    for ep in range(max_ep):
        ...
        for xb, xfb, yb, sb in loader:
            if dev != "cpu":
                xb, xfb, yb, sb = (xb.to(dev, non_blocking=True), xfb.to(dev, non_blocking=True),
                                   yb.to(dev, non_blocking=True), sb.to(dev, non_blocking=True))
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(v_xp), 1024):
                vout.append(model(v_xp[i:i + 1024], v_xf[i:i + 1024], v_s[i:i + 1024]))
    ...
    row["val_mae"] = metrics(to_mw(ds["val"]["y"], dmean, dstd),
                             to_mw(vp, dmean, dstd))["mae"]

    with torch.no_grad():
        tp = []
        te = ds["test"]
        t_xp, t_xf, t_s = [torch.from_numpy(np.asarray(te[k])).to(dev) for k in ("xp", "xf", "s")]
    ...
    row["test_ramp_viol"] = ramp_violation_rate(pred, te["xp"][:, -1, dp.IDX_DEMAND],
                                                dstd, ds["stats"]["ramp_mw"])
    idx = np.arange(len(te["y"]))[:3000]
    samp = {k: te[k][idx] for k in ("xp", "xf", "s")}
    row["test_negsens_rate"] = thermal_violation_rate(model, samp["xp"], samp["xf"], samp["s"],
                                                      ds["stats"], device=dev)
    if save_preds:
        os.makedirs(cfg.results_dir, exist_ok=True)
        np.savez(os.path.join(cfg.results_dir, f"preds_{name}_h{horizon}_s{seed}.npz"),
                 y=to_mw(te["y"], dmean, dstd), p=to_mw(pred, dmean, dstd),
                 t=te["t0"].asi8)
    row["train_sec"] = round(time.time() - t0, 1)
    row["val_mse_scaled"] = round(best_val, 5)
    if verbose:
        print(f"{name:15s} h={horizon:3d} s={seed} lt={lam_t} ls={lam_s} "
              ...
              f"[{row['train_sec']}s]", flush=True)
    return row


FIELDNAMES = ["model", "horizon", "seed", "lam_thermal", "lam_smooth", "tag",
              "train_sec", "val_mse_scaled", "val_mae", "test_mae", "test_rmse",
              "test_mae_peak", "test_mape", "test_smape", "test_negsens_rate",
              "test_ramp_viol"]
...
def done_keys(path):
    ...
    with open(path) as f:
        for r in csv.DictReader(f):
            try:
                keys.add((r["model"], int(float(r["horizon"])), int(float(r["seed"])),
                          float(r.get("lam_thermal", 0) or 0), float(r.get("lam_smooth", 0) or 0),
                          r.get("tag", "")))
...
def tuned_for(bp, name, horizon):
    ...
    if not entry:
        return {}, None, None
    key = str(horizon) if str(horizon) in entry else min(entry, key=lambda k: abs(int(k) - horizon))
    e = entry[key]
...
def run_all(cfg, quick=False, use_tuned=False):
    ...
    if use_tuned and not bp:
        print("warning: --use-tuned set but results/best_params.json missing; using defaults")
        ...
        for name in models:
            ...
            for seed in seeds:
                ...
                if (name, h, seed, lt, ls, tag) in done:
                    ...
                    continue
                r = run_one(cfg, name, h, seed, lt, ls, quick,
                            save_preds=(name in ("tide", "pg_tide") and h == 72), hp=hp)
                r["tag"] = tag
                rows.append(r)
                append_csv(out, [r])
    ...
    print(df[df.tag != "ablation"].pivot_table(index="model", columns="horizon",
                                               values="test_mae").round(1))


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--model", choices=["seasonal_naive", "lstm", "tide", "pg_tide", "pinn"])
    ...
    if a.all:
        ...
        r = run_one(cfg, a.model, a.horizon, a.seed, lt if a.model in PHYSICS_MODELS else 0.0,
                    ls if a.model in PHYSICS_MODELS else 0.0, a.quick, a.epochs, hp=hp)
        os.makedirs(cfg.results_dir, exist_ok=True)
