...
def metrics(y_mw, p_mw, peak_mask=None, prefix=""):
    ...
    out = {
        ...
        f"{prefix}smape": float(
            np.mean(2 * np.abs(e) / np.maximum(np.abs(y) + np.abs(p), 1.0)) * 100
        ),
    }
    if peak_mask is not None:
        m = np.asarray(peak_mask, bool)
        out[f"{prefix}mae_peak"] = (
            float(np.mean(np.abs(e[m]))) if m.any() else float("nan")
        )
    return out
...
def ramp_violation_rate(p_scaled, hist_last_scaled, dstd, ramp_mw):
    ...
    dd = (
        np.abs(
            np.concatenate([first[:, None], p_scaled[:, 1:] - p_scaled[:, :-1]], axis=1)
        )
        * dstd
    )
