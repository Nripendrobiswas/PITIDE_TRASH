...
def main():
    ...
    lines.append(
        f"missing values: {int(raw.isna().sum().sum())}, duplicated timestamps: {int(raw.index.duplicated().sum())}"
    )
    full = pd.date_range(raw.index.min(), raw.index.max(), freq="h")
    lines.append(f"missing hours (gaps): {len(full) - len(raw)}")
    lines.append(
        f"Generation(MW) identical to Demand(MW): {df.attrs['gen_eq_dem_rows']} rows "
        f"({df.attrs['gen_eq_dem_rows'] / len(raw) * 100:.1f}%) -> column unusable for a "
        f"power-balance constraint; modeling uses Demand(MW) only."
    )
    lines.append(
        f"demand spike rows cleaned (isolated glitches / implausible values): {df.attrs['outlier_demand_rows']}"
    )
    top = traw.diff().abs().sort_values(ascending=False).head(5)
    lines.append(
        "top raw demand jumps: "
        + "; ".join(f"{t} = {v:.0f} MW" for t, v in top.items())
    )
    lines.append(
        f"temperature: mean {raw[COL_T].mean():.1f} C, max {raw[COL_T].max():.1f} C; "
    ...
    binm = (
        pd.DataFrame({"T": df[COL_T], "d": df["demand_clean"]})
        .groupby(pd.cut(df[COL_T], 40), observed=True)["d"]
        .mean()
        .reset_index(name="d")
    )
    ...
    per_year = (
        (raw["Generation(MW)"] == raw["Demand(MW)"]).groupby(raw.index.year)
    ).mean() * 100
